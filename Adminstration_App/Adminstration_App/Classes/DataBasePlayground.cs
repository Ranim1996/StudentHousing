...
namespace Administration_App.Classes
{
    ...
    public class DataBasePlayground
    {
        ...
        public static void AddAnnouncement(string user, string topic, string text, string date1, string date2, string status)
        {
            using (IDbConnection connection = new System.Data.SqlClient.SqlConnection(ConStr("loginsDB")))
            {
                connection.Execute($"INSERT INTO announcements (pWSA, Topic, Text, DateA, DateE, Status) VALUES ('{user}', '{ topic }', '{ text }', '{ date1 }', '{ date2 }', '{ status }' );");
            }
        }
        //--------------------------------------------------------------------------
